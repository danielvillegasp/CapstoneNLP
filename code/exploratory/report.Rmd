...
rules i.e. syntax, grammar, etc. and together are meaningful to other humans. Even though,
...
### Document Summary
The number of lines per document i.e. tweets, news and blogs. was limited to 10000. Here, a 
brief summary of the documents is presented.
...
```{r ngramCnt, message=FALSE, warning=FALSE}
unigrams <- data.frame(unigram=unlist(sapply(tk_corp, function(x){
  ...
  ungroup() %>%
  mutate(w0=as.character(unigram), p = cnt / sum(cnt)) %>%
  select(w0, cnt, p) %>%
...
bigrams <- data.frame(bigram=unlist(sapply(bigram_corp, function(x){
  ...
  dplyr::summarise(cnt=n()) %>%
  ungroup() %>%
  inner_join(y=unigrams, by='w0') %>%
  mutate(p = cnt.x / cnt.y) %>%
  dplyr::ungroup() %>%
  dplyr::mutate(cnt=cnt.x) %>%
  dplyr::select(w0, w1, cnt, p) %>%
...
trigrams <- data.frame(trigram=unlist(sapply(trigram_corp, function(x){
  ...
  dplyr::summarise(cnt=n()) %>%
  ungroup() %>%
  inner_join(y=bigrams, by=c("w0" = "w0", "w1" = "w1")) %>%
  mutate(p = cnt.x / cnt.y) %>%
  dplyr::ungroup() %>%
  dplyr::mutate(cnt=cnt.x) %>%
  dplyr::select(w0, w1, w2, cnt, p) %>%
  dplyr::arrange(desc(p)) 
```
...
```{r bigHist}
ggplot(bigrams , aes(x=p)) + geom_histogram(bins=40)
```
...
```{r trigHist}
ggplot(trigrams , aes(x=p)) + geom_histogram(bins=40)
```
...
```{r}
bigrams2 <- bigrams[c(sample(1:1000, 500), 
                      sample(1001:nrow(bigrams), 500)),]
...
```{r}
bigram_graph <- graph.data.frame(bigrams %>% 
                                   mutate(weight=p) %>%
                                   select(w0,w1,weight), directed = T)
```
...
```{r}
  ggplot(inner_join(btnc, unigrams %>% mutate(Word=w0), by='Word'),
...
* A more efficient way of handling the large datasets will have to be implemented for the shiny 
app.
* A strong relation between centrality measure of nodes from the bigram graph and the
unigram probabilities was found. Such relation shows that in a way, the set of all
bigrams preserves information about the set of all unigrams. The relationship found and other 
relations based on graph measures might be useful to create new features for the 
learning algorithm.
